...
#import "AFGoogleMapsHelper.h"

static NSString * const baseURL = @"";

@implementation AFGoogleMapsHelper
#pragma mark - sharedClient

+ (instancetype)sharedAFGoogleMapsHelper {
    static AFGoogleMapsHelper *singleton;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        
        singleton=[[self alloc] initWithBaseURL:[NSURL URLWithString:baseURL]];
    
    });
    
//
    singleton.APIKey = @"AIzaSyCndH1E3l7XI5RMWHXn6KqI6163SPcKoJs"; // Obtained from Google
    return singleton;
}
...
- (instancetype)initWithBaseURL:(NSURL *)url
{
    ...
    if (self) {
        ...
        //NSString *email = [[NSUserDefaults standardUserDefaults] objectForKey:kUserDefaultEmail];
...
- (void)geocodeAddressString:(NSString *)query components:(NSDictionary *)components completionHandler:(SPGoogleGeoCoderCompletionBlock)completion
{
    ...
    [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] cancelAllRequests];
    
    [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] GET:@"http://maps.googleapis.com/maps/api/geocode/json" parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
        //
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        SPGoogleGeoCoderResponse responseCode = [AFGoogleMapsHelper responseCodeFromJSONResponse:responseObject];
        if (responseCode == SPGoogleGeoCoderResponseOK || responseCode == SPGoogleGeoCoderResponseOKZeroResults) {
            dispatch_async(dispatch_get_main_queue(), ^{
                MOGoogleGeocodeList *geocode = [MOGoogleGeocodeList modelObjectWithDictionary:responseObject];
                completion(geocode, responseCode, [AFGoogleMapsHelper messageStringFromResponseCode:responseCode]);
            });
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(nil, responseCode, [AFGoogleMapsHelper messageStringFromResponseCode:responseCode]);
            });
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            completion(nil, error.code, [AFGoogleMapsHelper messageStringFromResponseCode:error.code]);
        });
    }];
...
- (void)reverseGeocodeCoordinate:(CLLocationCoordinate2D)coordinate resultTypes:(NSArray *)resultTypes locationTypes:(NSArray *)locationTypes completionHandler:(SPGoogleGeoCoderCompletionBlock)completion
{
    ...
    [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] cancelAllRequests];
    [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] GET:@"http://maps.googleapis.com/maps/api/geocode/json" parameters:params
    progress:^(NSProgress * _Nonnull downloadProgress) {
        //
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        SPGoogleGeoCoderResponse responseCode = [AFGoogleMapsHelper responseCodeFromJSONResponse:responseObject];
        if (responseCode == SPGoogleGeoCoderResponseOK || responseCode == SPGoogleGeoCoderResponseOKZeroResults) {
            MOGoogleGeocodeList *googleGeoCodeList = [MOGoogleGeocodeList modelObjectWithDictionary:responseObject];
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(googleGeoCodeList, responseCode, [AFGoogleMapsHelper messageStringFromResponseCode:responseCode]);
            });
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(nil, responseCode, [AFGoogleMapsHelper messageStringFromResponseCode:responseCode]);
...
- (void)getDirections:(CLLocationCoordinate2D)coordinateSource andCoordinateDestination:(CLLocationCoordinate2D)coordinateDestination  andDrawPoints:(SPGoogleDirectionsVoidBlock)drawPoints andPlaceMarks:(SPGoogleDirectionsPlaceMarksBlock)placeMarks
{
    [SVProgressHUD show];
    [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] cancelAllRequests];
    [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] GET:[NSString stringWithFormat:@"http://maps.googleapis.com/maps/api/directions/json?origin=%@&destination=%@&mode=driving",[NSString stringWithFormat:@"%f,%f", coordinateSource.latitude, coordinateSource.longitude],[NSString stringWithFormat:@"%f,%f", coordinateDestination.latitude, coordinateDestination.longitude]] parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        //
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
          [SVProgressHUD dismiss];
            if ([[responseObject utilities_objectForKeyNotNull:@"status"] isEqualToString:@"ZERO_RESULTS"])
            {
                [SVProgressHUD showErrorWithStatus:@"Failed to get directions"];
                return;
            }
            NSArray *routeArray = [responseObject utilities_objectForKeyNotNull:@"routes"];
            ...
            if (route) {
                [self parseResponse:responseObject andDrawPoints:drawPoints andPlaceMarks:placeMarks andPolyLineSetArray:polylineSetAry andDirectionsSetArray:directionStrAry andDistanceSetArray:distanceStrAry];
            }
...
- (void)getAutoCompleteFromGoogle:(NSString *)keyWord andAutoComplete:(SPGoogleAutoCompletionBlock)autocomplete{
    [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] cancelAllRequests];
    [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] GET:[NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/autocomplete/json?input=%@&key=%@",[keyWord utilities_urlEncode], [[AFGoogleMapsHelper sharedAFGoogleMapsHelper] APIKey]] parameters:nil
                                 progress:^(NSProgress * _Nonnull downloadProgress) {
                                     //
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        
        MOGoogleAutoCompleteList *autocompleteList = [MOGoogleAutoCompleteList modelObjectWithDictionary:responseObject];
        SPGoogleGeoCoderResponse responseCode = [AFGoogleMapsHelper responseCodeFromJSONResponse:responseObject];
        autocomplete(autocompleteList,responseCode,[AFGoogleMapsHelper messageStringFromResponseCode:responseCode]);
                                     
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            
            dispatch_async(dispatch_get_main_queue(), ^{
                autocomplete(nil, error.code, [AFGoogleMapsHelper messageStringFromResponseCode:error.code]);
            });
