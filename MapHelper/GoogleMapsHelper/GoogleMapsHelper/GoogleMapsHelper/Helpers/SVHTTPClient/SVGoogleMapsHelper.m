...
#import "SVGoogleMapsHelper.h"

@implementation SVGoogleMapsHelper
#pragma mark - sharedClient

+ (instancetype)sharedGoogleMapHelper {
    static SVGoogleMapsHelper *singleton;
    singleton = [SVGoogleMapsHelper sharedClientWithIdentifier:NSStringFromClass([self class])];
...
- (void)geocodeAddressString:(NSString *)query components:(NSDictionary *)components completionHandler:(SPGoogleGeoCoderCompletionBlock)completion
{
    ...
    [[SVGoogleMapsHelper sharedGoogleMapHelper] cancelAllRequests];
    
    [[SVGoogleMapsHelper sharedGoogleMapHelper] GET:@"http://maps.googleapis.com/maps/api/geocode/json" parameters:params completion:^(id response, NSHTTPURLResponse *urlResponse, NSError *error) {
        [SVProgressHUD dismiss];
        SPGoogleGeoCoderResponse responseCode = [SVGoogleMapsHelper responseCodeFromJSONResponse:response];
        if (responseCode == SPGoogleGeoCoderResponseOK || responseCode == SPGoogleGeoCoderResponseOKZeroResults) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                completion(geocode, responseCode, [SVGoogleMapsHelper messageStringFromResponseCode:responseCode]);
            });
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(nil, responseCode, [SVGoogleMapsHelper messageStringFromResponseCode:responseCode]);
            });
        }
    }];
}
...
- (void)reverseGeocodeCoordinate:(CLLocationCoordinate2D)coordinate resultTypes:(NSArray *)resultTypes locationTypes:(NSArray *)locationTypes completionHandler:(SPGoogleGeoCoderCompletionBlock)completion
{
    ...
    [[SVGoogleMapsHelper sharedGoogleMapHelper]  GET:@"http://maps.googleapis.com/maps/api/geocode/json" parameters:params completion:^(id response, NSHTTPURLResponse *urlResponse, NSError *error) {
        ...
        if (responseCode == SPGoogleGeoCoderResponseOK || responseCode == SPGoogleGeoCoderResponseOKZeroResults) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(googleGeoCodeList, responseCode, [SVGoogleMapsHelper messageStringFromResponseCode:responseCode]);
            });
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
                completion(nil, responseCode, [SVGoogleMapsHelper messageStringFromResponseCode:responseCode]);
...
- (void)getDirections:(CLLocationCoordinate2D)coordinateSource andCoordinateDestination:(CLLocationCoordinate2D)coordinateDestination  andDrawPoints:(SPGoogleDirectionsVoidBlock)drawPoints andPlaceMarks:(SPGoogleDirectionsPlaceMarksBlock)placeMarks
{
    [SVProgressHUD show];
    [[SVGoogleMapsHelper sharedGoogleMapHelper] cancelAllRequests];
    [[SVGoogleMapsHelper sharedGoogleMapHelper]  GET:[NSString stringWithFormat:@"http://maps.googleapis.com/maps/api/directions/json?origin=%@&destination=%@&mode=driving",[NSString stringWithFormat:@"%f,%f", coordinateSource.latitude, coordinateSource.longitude],[NSString stringWithFormat:@"%f,%f", coordinateDestination.latitude, coordinateDestination.longitude]] parameters:nil completion:^(id response, NSHTTPURLResponse *urlResponse, NSError *error) {
        [SVProgressHUD dismiss];
        if (!error) {
...
- (void)getAutoCompleteFromGoogle:(NSString *)keyWord andAutoComplete:(SPGoogleAutoCompletionBlock)autocomplete{
    [SVProgressHUD show];
    [[SVGoogleMapsHelper sharedGoogleMapHelper] cancelAllRequests];
    [[SVGoogleMapsHelper sharedGoogleMapHelper] GET:[NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/autocomplete/json?input=%@&key=%@",[keyWord utilities_urlEncode], [[SVGoogleMapsHelper sharedGoogleMapHelper] APIKey]] parameters:nil completion:^(id response, NSHTTPURLResponse *urlResponse, NSError *error) {
        [SVProgressHUD dismiss];
        MOGoogleAutoCompleteList *autocompleteList = [MOGoogleAutoCompleteList modelObjectWithDictionary:response];
        SPGoogleGeoCoderResponse responseCode = [SVGoogleMapsHelper responseCodeFromJSONResponse:response];
        autocomplete(autocompleteList,responseCode,[SVGoogleMapsHelper messageStringFromResponseCode:responseCode]);
    }];
}
